...
export async function submitPrediction(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    ...
    if (typeof prediction !== 'number') return res.status(400).json({ error: 'Prediction must be a number' });
    ...
    if (currentRound.phase !== 'PREDICTION') return res.status(400).json({ error: 'Not in prediction phase' });
    ...
    const predictionsCount = await prisma.prediction.count({ where: { roundId: currentRound.id } });
    ...
    if (predictionsCount >= totalPlayers) {
        ...
        const playersBySeat = [...game.room.players].sort((a: any, b: any) => (a.seat || 0) - (b.seat || 0));
        const dealerIndex = playersBySeat.findIndex(p => p.id === currentRound.dealerId);
...
async function finishRound(gameId: string, roundId: string) {
    ...
    // Fetch players
    const roomPlayers = await prisma.roomPlayer.findMany({ where: { roomId: game.roomId } });
    ...
    if (round.number >= game.totalRounds) {
        await prisma.game.update({ where: { id: gameId }, data: { state: 'FINISHED', finishedAt: new Date() } });
        ...
        const sortedPlayers = [...roomPlayers].sort((a,b) => (a.seat||0) - (b.seat||0));
        ...
        await prisma.game.update({ where: { id: gameId }, data: { currentRound: nextRoundNum } });
        await prisma.round.update({ where: { id: roundId }, data: { phase: 'DONE' } });
    }
...
export async function playCard(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    const gameId = req.params.id;
    const userId = req.user?.sub;
    ...
    if (!game) return res.status(404).json({ error: 'Game not found' });
    const currentRound = game.rounds[0];
    if (!currentRound || currentRound.phase !== 'PLAY') return res.status(400).json({ error: 'Not in Play phase' });

    const currentTrick = currentRound.tricks[0];
    if (!currentTrick) return res.status(400).json({ error: 'No active trick' });

    // Determine whose turn it is
    const players = [...game.room.players].sort((a: any, b: any) => (a.seat || 0) - (b.seat || 0));
    ...
    if (player.id !== expectedPlayerId) return res.status(400).json({ error: 'Not your turn' });

    // Check if card is in hand
    const hand = currentRound.hands.find(h => h.playerId === player.id);
    const handCard = hand?.cards.find(hc => hc.id === cardId); 
    const targetHandCard = handCard || hand?.cards.find(hc => hc.cardId === cardId);

    if (!targetHandCard) return res.status(400).json({ error: 'Card not in hand' });
    ...
    await prisma.play.create({
        data: {
            ...
            playChoice: (targetHandCard.card.cardType === 'TIGRESS' && choice) ? choice : undefined
        }
    });
...
export async function collectTrick(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    const gameId = req.params.id;
    ...
    if (currentTrick.index >= currentRound.handSize) {
        ...
        finishRound(game.id, currentRound.id).catch(err => console.error('finishRound error', err));
    } else {
        // Next Trick
        await prisma.trick.create({
            data: {
